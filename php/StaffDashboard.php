...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cancel_exams'])) {
    ...
    if (!empty($selectedExams)) {
        ...
        $stmt = $db->prepare(
            "UPDATE exams SET teacher_id = NULL WHERE exam_id IN ($placeholders) AND teacher_id = ?"
        );
...
// Fetch exams where teacher_id matches logged-in staff and include current student counts
$sql = "SELECT e.exam_id, e.exam_name, e.campus, e.room_number, e.exam_date, e.exam_time, e.capacity,
           COUNT(DISTINCT r.student_id) AS student_count
    FROM exams e
    LEFT JOIN registrations r ON e.exam_id = r.exam_id
    WHERE e.teacher_id = ?
    GROUP BY e.exam_id, e.exam_name, e.campus, e.room_number, e.exam_date, e.exam_time, e.capacity";
$stmt = $db->prepare($sql);
$stmt->execute([$userId]);
...
<head>
    ...
    <style>
        ...
        @media print {
            .logout-btn, .print-btn, .cancel-btn, .select-all-container, .exam-checkbox, .button-container {
...
<body>
    ...
    <div class="dashboard-container">
        <div class="welcome-card">
            ...
            <div style="display:inline-block">
                ...
                <button class="print-btn" id="printBtn" title="Print"><span style="font-size:1.05em"></span> 🖨️</button>
                <button class="refresh-btn" id="refreshBtn" title="Refresh counts">REFRESH LIST</button>
            </div>
        </div>
                        ...
                        <tbody>
                            <?php foreach ($exams as $exam): ?>
                                <tr class="clickable-row">
                                    ...
                                    <td>
                                        <?php $count = isset($exam['student_count']) ? (int)$exam['student_count'] : 0; ?>
                                        <?php
                                            $cap = isset($exam['capacity']) ? (int)$exam['capacity'] : 0;
                                            $badgeClass = 'count-badge';
                                            if ($cap > 0) {
                                                if ($count >= $cap) $badgeClass .= ' full';
                                                elseif ($count >= max(0, $cap - 2)) $badgeClass .= ' nearfull';
                                            }
                                        ?>
                                        <span class="<?= $badgeClass ?>"><?= htmlspecialchars($count) ?> / <?= htmlspecialchars($cap) ?></span>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
    ...
    <script>
        ...
        // Refresh counts handler
        document.getElementById('refreshBtn').addEventListener('click', function() {
            // Simple full reload to refresh counts from the server
            location.reload();
        });
